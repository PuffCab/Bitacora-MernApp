...
function Friends({ user }) {
  ...
  console.log(`userPofile en FRIENDS`, user);

  const [friends, setFriends] = useState([]);
  const { loggedUser, userUpdated, UpdateUser } = useContext(AuthContext);
  const [isFriend, setIsFriend] = useState(false);

  console.log(`LOGGEDuser in FRIENDS>>>>>`, loggedUser);
  useEffect(() => {
    setIsFriend(loggedUser.friends.includes(user?._id));
  }, [loggedUser, user._id]);
  ...
  const handleClick = async () => {
    try {
      if (isFriend) {
        await axios2.put("/users/" + user._id + "/unfollow", {
          userId: loggedUser._id,
        });
        UpdateUser(loggedUser._id);
        console.log("UNFOLLED user with ID", user._id);
      } else {
        await axios2.put("/users/" + user._id + "/follow", {
          userId: loggedUser._id,
        });
        UpdateUser(loggedUser._id);
    ...
    <>
      ...
      <div className="friendsFollowings">
        {user.userName !== loggedUser.userName && (
          <button className="friendsFollowButton" onClick={handleClick}>
          ...
          >
            ...
            <div className="friendsFollowing">
              <img
                src={
                  friend.coverPicture
                    ? testImgFolder + friend.coverPicture
                    : testImgFolder + "/user/avatar.jpeg"
                }
                alt=""
                className="friendsFollowingImg"
              />
